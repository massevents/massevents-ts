...
import { STORY_PREPARED, UPDATE_STORY_ARGS, RESET_STORY_ARGS, STORY_ARGS_UPDATED, STORY_CHANGED, SELECT_STORY, SET_STORIES, STORY_SPECIFIED } from '@storybook/core-events';
...
export const init = ({
  ...
  const api = {
    ...
    fetchStoryList: async () => {
      try {
        const storyIndex = await indexClient.fetch(); // We can only do this if the stories.json is a proper storyIndex
  ...
  const initModule = async () => {
    ...
    fullAPI.on(SELECT_STORY, function handler(_ref) {
      let {
        ...
        storyId
          ...
          rest = _objectWithoutPropertiesLoose(_ref, ["kind", "story", "storyId"]);
    ...
    fullAPI.on(STORY_PREPARED, function handler(_ref2) {
      let {
        id
      } = _ref2,
          update = _objectWithoutPropertiesLoose(_ref2, ["id"]);
